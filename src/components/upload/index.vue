<template>
  <div class="uploader-box">
    <el-upload class="avatar-uploader" :action="`${baseURL}${action}`" :before-upload="beforeUpload"
      :on-remove="handleRemove" :on-success="handleSuccess" :on-error="handleError" :data="upObj" :headers="headers"
      :accept="accept" :show-file-list="false" :name="name" :multiple="multiple" v-if="limit == 1">
      <template v-if="uploadText">
        <el-button type="primary" :size="btnSize">{{ uploadText }}<i
    ...
    <el-upload class="avatar-uploader" list-type="picture-card" :action="`${baseURL}${action}`"
      :before-upload="beforeUpload" :on-preview="handlePictureCardPreview" :on-remove="handleRemove"
      :on-success="handleSuccess" :on-error="handleError" :data="upObj" :headers="headers" :accept="accept"
      :file-list="value" :name="name" :limit="limit" :multiple="multiple" :on-exceed="exceed" v-else>
...
export default {
  ...
  methods: {
    ...
    beforeUpload(file) {
      ...
      if (!isLt8M) {
        this.$message({
          message: that.$t('components.message4'),
          type: 'error'
        })
        return false
      }
    ...
    handleSuccess(res, file, fileList) {
      ...
      if (res.code != 200) {
        file.status = 'error'
        this.$message.error(`${that.$t('components.uploadImgError')}:${res.data.message}, ${that.$t('components.deleteImgUpload')}`)
      } else {
        if (this.$listeners['onSuccess']) {
          ...
          this.$message({
            message: that.$t('components.message5'),
            type: 'success'
          })
        }
      }
    ...
    handleError(error) {
      ...
      this.$message({
        message: err.message || that.$t('components.uploadError'),
        type: 'error'
      })
    },
    ...
    exceed(files, fileList) {
      this.$message.error(`${this.$t('components.maxNum')}${this.limit}${this.$t('components.files')}`)
    },
