    ...
    <div class="p-15 bg-white">
      ...
      <el-table class="ptd-5" id="list_table" ref="list_table" v-loading="listLoading" :data="list"
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="$router.push({path: `/adver/weChatList?id=${scope.row.id}&name=${scope.row.name}`})">设置广告</el-button>
            <el-button type="primary" size="mini" round plain @click="setShow(scope.row)"
              v-if="isBrand()">{{ scope.row.is_show == 1 ? '已开启' : '已关闭' }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
