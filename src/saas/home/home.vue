    ...
    <el-row :gutter="device === 'mobile' ? 10 : 20" class="mt-15">
      <el-col :xs="24" :sm="16" :lg="18">
          <el-scrollbar class="bg-white">
            <div class="flex align-center brand-box white-space">
              <div class="pl-10 pr-10 cursor" :class="{'text-primary': !brandId}"  @click="brandId = ''; loadStat()">全部品牌</div>
              <div class="pl-10 pr-10 cursor" v-for="item in brandTop" :class="{'text-primary': brandId == item.id}"  @click="brandId = item.id; loadStat()">{{ item.companyName }}</div>
            </div>
          </el-scrollbar>
      </el-col>
