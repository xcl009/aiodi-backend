    ...
    <el-row :gutter="device === 'mobile' ? 10 : 20">
      ...
      <el-col :xs="24" :sm="12" :lg="6">
        <div class="o-v p-15 card-panel cursor bg-white" @click="$router.push({path: `/order`})">
          ...
          <div class="flex align-center line-1">
            <div class="flex1 flex align-center">
              <div class="mr-10 fs-b5"><count-to :start-val="0" :end-val="delComma(orderStat.orderNumber)" :duration="2600" :decimals="2"/></div>
              <i class="iconfont icon-right fs-s1"></i>
            </div>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       */
      getQuerHistogram() {
        this.$get('iot-saas-order/admin/order/count/querHistogram').then(res => {
