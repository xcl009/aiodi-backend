    ...
    <condition ref="condition" :clickSubmit="clickSubmit" @reset="reset" @query="toQuery">
      ...
      <template v-slot:endButton>
        <el-button type="primary" size="small" @click="$router.push({ path: `/shopping/productEdit` })"><i
            class="el-icon-circle-plus-outline el-icon--left" />{{ $t('public.newEquipmentAdded') }}</el-button>
      </template>
    </condition>
...
export default {
  ...
  methods: {
    ...
    undChange(item, index) {
      let that = this;
      let text = item.status == 1 ? that.$t('shopping.message') : that.$t('shopping.message1');
      this.$confirm(text, that.$t('public.message'), {
        confirmButtonText: that.$t('public.confirm'),
        cancelButtonText: that.$t('public.cancel'),
        type: 'warning'
      }).then(() => {

        this.$post(`iot-saas-order/admin/product/upDown/${item.id}/${item.status == 1 ? 0 : 1}`, {}).then((
          res = {}) => {
          this.$message({
          ...
          this.$message({
            message: that.$t('shopping.operationError'),
            type: 'success'
          })
        })
      })
