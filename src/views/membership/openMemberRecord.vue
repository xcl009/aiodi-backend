  ...
  <div>
    <condition ref="condition" @reset="reset" @query="toQuery">
      <template v-slot:defult>
        <el-select v-model="form.state" :placeholder="$t('public.status')" @change="toQuery()">
          <el-option :label="item.title" :value="item.val"
            v-for="(item, key) in [{ val: '', title: $t('public.all') }, { val: 'O', title: $t('membership.paid') }, { val: 'U', title: $t('public.obligation') }]" />
        </el-select>
        <el-input v-model="form.serviceName" :placeholder="$t('membership.serviceName')" />
        <selectSearch v-model="form.userId" :type="1" name="mobile" :placeholder="$t('public.phone')" @change="toQuery()">
        </selectSearch>
    ...
    <div class="pl-15 pr-15 pb-5 bg-white">
      <el-table class="ptd-5" id="list_table" ref="list_table" v-loading="listLoading" :data="list"
        ...
        <el-table-column :label="$t('membership.oddNumbers')">
          <template slot-scope="scope">
            <div>{{ scope.row.orderNo || '--' }}</div>
          </template>
        </el-table-column>
        <el-table-column :label="$t('membership.cardholder')">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('membership.createTime')">
          <template slot-scope="scope">
        ...
        <el-table-column :label="$t('membership.orderAmount')">
          <template slot-scope="scope">
            <div class="mb-5">{{ scope.row.orderAmount || '--' }}</div>
          </template>
        </el-table-column>
        <el-table-column :label="$t('membership.cardDeposit')">
          <template slot-scope="scope">
            <div class="mb-5">{{ scope.row.depositAmount || '--' }}</div>
          </template>
        </el-table-column>
        <el-table-column :label="$t('membership.storeName')">
          <template slot-scope="scope">
            {{ scope.row.storeName || '--' }}
          </template>
        </el-table-column>
        <el-table-column :label="$t('public.orderType')">
          <template slot-scope="scope">
          ...
          <template slot-scope="scope">
            <el-button type="info" size="mini" plain @click="setRows(1, scope.row, 1)"
              v-if="scope.row.state == 'O' && scope.row.depositAmount > 0 && scope.row.amountRefund == 0">{{
                $t('order.refundOfDeposit') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog :visible.sync="dialogStatus" :center="true" :show-close="false" width="454px">
      ...
      <template v-if="dialogType == 1">
        <div class="text-center">
          <div class="text-black">{{ $t('membership.text1') }}</div>
          <div class="mt-10 pl-40 pr-40 text-gray">{{ $t('membership.text2') }}</div>
        </div>
      </template>
      ...
      switch (this.dialogType) {
        case 1:
          ...
          this.$post('iot-saas-order/api/brand/goods/order/refundDeposit', {
            ...
            this.$message({
              type: 'success',
              message: that.$t('membership.refundSuccessful')
            })
