    ...
    <drawer ref="filterDrawer">
      <template v-slot:defult>
        ...
        <el-form-item>
          ...
          <el-button type="primary" plain @click="outTab('list_table', user_type == 1 ? '代理商提现记录' : '商户提现记录')">导出<i class="el-icon-male el-icon--right" /></el-button>
        </el-form-item>
      </template>
      <template v-slot:more>
        ...
        <el-form-item label="提现ID:" class="hidden-lg-and-up">
    ...
    <div class="p-5">
      <div class="flex bg-white">
        <div class="flex1">
          <el-tabs v-model="listQuery.withdraw_status" class="mb-0" type="card" @tab-click="typeClick">
            ...
            <el-tab-pane :label="`返现中(${numInfo.drawbacking_num || 0})`" name="5"/>
            <el-tab-pane :label="`已到账(${numInfo.sended_num || 0})`" name="6"/>
          </el-tabs>
        </div>
        <div class="pl-15 tabs-right fs-s3">
          <span class="mr-15">总提现：{{ stat_data.total_apply_amount || 0 }}元</span>
          <span class="mr-15">代理商手续费：{{ stat_data.upper_fee_amount || 0 }}元</span>
      ...
      <div class="bg-white">
        ...
        <el-table class="ptd-5" id="list_table" ref="list_table" v-loading="listLoading" :data="list" :max-height="tableMaxH" element-loading-text="Loading" stripe highlight-current-row>
          ...
          <el-table-column label="商户名称" width="150" v-if="user_type == 2">
            <template slot-scope="scope">
              {{ scope.row.store_name }}
            </template>
          </el-table-column>
          <el-table-column label="上级用户" width="150">
            <template slot-scope="scope">
              <div class="flex align-center">
                <el-avatar size="small" :src="scope.row.upper_review_avatar"></el-avatar>
                <div class="flex1 ml-5">{{ scope.row.upper_review_name }}<br>{{ scope.row.upper_review_phone }}</div>
              </div>
            </template>
          </el-table-column>
          <el-table-column :label="user_type == 1 ? '代理商' : '商户联系人'" width="150">
            <template slot-scope="scope">
              <div class="flex align-center">
          ...
          <el-table-column label="提现时间" width="100">
            <template slot-scope="scope">
          ...
          <el-table-column label="上级审核">
            <template slot-scope="scope">{{ scope.row.upper_review_status_des }}</template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="150">
            <template slot-scope="scope">
              <div v-if="(agentInfo.agent_level != 1 && scope.row.withdraw_status == 0) || (agentInfo.agent_level == 1 && (scope.row.withdraw_status == 0 || scope.row.withdraw_status == 3))">
...
<script>
  ...
  export default {
    ...
    mounted(options) {
      ...
      this.getNum()
      this.getList()
      this.$store.dispatch('user/postSetRecode', [1, 1, this.user_type])
    },
    methods: {
      /**
       * 数量
      ...
      getNum() {
        ...
        this.$get('agentapi/agent_withdraw_applys_stat_num', listQuery).then(res => {
          this.numInfo = res
        })
      },
      ...
      getList() {
        ...
        if(listQuery.withdraw_status == 2 && this.checkRoles(['partner','terminal'])){
          listQuery.withdraw_status = 4
        }
       ...
       */
      postApply(row, agree){
        let url = 'agentapi/upper_review_apply'
        if (agentInfo.agent_level == 1) url = 'agentapi/final_review_apply'
        this.$post(url, {
          apply_id: row.id,
          agree: agree
        }).then(res => {
          ...
          if (agentInfo.agent_level == 1){
            if(row.withdraw_type == 0 || row.withdraw_type == 3 || row.withdraw_type == 4){
              row.withdraw_status = (agree == 0 ? 4 : 6)
            } else {
              row.withdraw_status = (agree == 0 ? 4 : 5)
            }
          } else {
