          ...
          <template v-slot:tabs>
            ...
            <div class="mb-10 flex align-center bg-white">
              ...
              <el-tabs class="flex-1" v-model="listQuery.payType" @tab-click="toQuery()">
                ...
                <el-tab-pane :key="index" :name="JSON.stringify(item.payType)" v-for="(item, index) in payChannellist"
                  v-if="index != 0">
      ...
      <div v-if="showColumn.length > 0">
        <el-table class="ptd-5" id="list_table" ref="list_table" v-loading="listLoading" :data="list"
          ...
          <template v-for="item in showColumn">
            ...
            <el-table-column :label="item.name" width="110" v-else-if="item.val && item.key == 'userMobile'">
              <template slot-scope="scope">
            ...
            <el-table-column :label="item.name" width="140" v-else-if="item.val && item.key == 'PayType'">
              <template slot-scope="scope">
                <div class="flex align-center">
                  <el-avatar :src="payChannel[scope.row.payType].logo" :size="25" v-if="payChannel[scope.row.payType]"
                    class="mr-5 radius-15"></el-avatar>
                  <span v-if="scope.row.orderAmount > 0 && scope.row.feeType == 3 && (isBrand() || isSaas())">({{
            ...
            <el-table-column :label="item.name" width="120" v-else-if="item.val && item.key == 'feeMode'">
              <template slot-scope="scope">
                <el-tooltip :content="showFeeMode(scope.row.feeType, scope.row.feeMode, 2)" placement="top">
                  <div v-if="scope.row.modeType != 'STEPBILLING'">{{ showFeeMode(scope.row.feeType, scope.row.feeMode, 1, scope.row.deviceTypeCode) }}</div>
                  <div v-else>
...
export default {
  ...
  methods: {
    ...
    getSystemTimeInChina(dates) {
      ...
      // const date = new Date();
      // const options = { timeZone: 'Asia/Ho_Chi_Minh', timeStyle: 'short' };
