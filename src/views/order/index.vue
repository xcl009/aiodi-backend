					...
					<div class="pl-20 pr-20 text-black">
						...
						<div class="flex pb-20 l-b">
							...
							<div>
								<div class="pl-20 pb-10 l-b-dashed">
									...
									<div class="flex mb-10">
										<div class="label-text" v-if="curRow.deviceTypeCode.indexOf('PA') > -1">{{ $t('public.sn') }}:</div>
								...
								<div class="pl-20 mt-10">
									...
									<div class="flex">
										...
										<div>
											<span class="mr-5"
												v-if="curRow.afterLevel >= 0 || curRow.level >= 0">{{ curRow.afterLevel ?
                        $t('order.powerConsumption') : $t('order.DuringLease') }}({{ curRow.afterLevel >= 0 ? curRow.afterLevel : curRow.level }}%)</span>
											<template v-if="curRow.freeTime > 0">
												<span class="mr-5"
													v-if="curRow.freeUser == 1">{{ $t('public.freeQuota') }}：{{ curRow.freeTime }}{{ $t('public.minute') }}</span>
												<span class="mr-5"
													v-else-if="curRow.freeUser == 3">{{ $t('order.suspendBilling') }}：{{ curRow.freeTime }}{{ $t('public.minute') }}</span>
	...
	export default {
		...
		methods: {
			...
			getStatNum() {
				...
				if (this.lowerAgent != 'ALL') params.type = this.lowerAgent ? 1 : 0
