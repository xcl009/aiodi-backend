	...
	<div>
		...
		<div class="pl-10 pr-10 bg-white">
			...
			$t('order.failedOrdersNum') }}</div>
				<div class="ml-20 text-primary cursor line-1" @click="setRows(6, {})" v-if="isSaas()">{{
			$t('order.leaseOrderNum')
		}}</div>
			...
			<div v-if="showColumn.length > 0">
				<el-table class="ptd-5" id="list_table" ref="list_table" v-loading="listLoading" :data="list"
					...
					<template v-for="item in showColumn">
						<el-table-column :label="item.name" width="170" v-if="item.val && item.key == 'userNickName'">
							<template slot-scope="scope">
								<div class="cursor text-blue" @click="setRows(1, scope.row, 4)"
									v-if="scope.row.userId == 0">
									{{ $t('order.usingUsers') }}
								</div>
								<div v-else class="flex align-center">
									<el-avatar :src="scope.row.userAvatar || agentInfo.avatar" class="mr-5 radius-15"
										:size="30"></el-avatar>
						...
						<el-table-column :label="item.name" width="110"
							v-else-if="item.val && item.key == 'userMobile'">
							<template slot-scope="scope">
						...
						<el-table-column :label="item.name" min-width="180"
							v-else-if="item.val && item.key == 'storeName'">
							<template slot-scope="scope">
			...
			"--" }}</div>
						...
						<el-table-column :label="item.name" width="50" v-else-if="item.val && item.key == 'sourceType'">
							<template slot-scope="scope">
								<el-tooltip
									:content="Constant.SourceType ? Constant.SourceType[scope.row.sourceType] : '--'">
						...
						<el-table-column :label="item.name" width="140" v-else-if="item.val && item.key == 'PayType'">
							<template slot-scope="scope">
								<div class="flex align-center">
									<el-avatar :src="payChannel[scope.row.payType].logo" :size="25"
										v-if="payChannel[scope.row.payType]" class="mr-5 radius-15"></el-avatar>
									<span
										v-if="scope.row.orderAmount > 0 && scope.row.feeType == 3 && (isBrand() || isSaas())">({{
			formatCurrency(scope.row.orderAmount) }})</span>
								</div>
							</template>
						</el-table-column>
						<el-table-column :label="item.name" width="160"
							v-else-if="item.val && item.key == 'chargeStartTime'">
							<template slot-scope="scope">
              ...
              <template slot-scope="scope">
                {{ showFeeName(scope.row.feeType) }}<span v-if="scope.row.feeType == 3">({{ scope.row.amountPaid }}元)</span>
						...
						<el-table-column :label="item.name" width="75"
							v-else-if="item.val && item.key == 'amountRefund'">
							<template slot-scope="scope">
						...
						<el-table-column :label="item.name" width="90" v-else-if="item.val && item.key == 'status'">
							<template slot-scope="scope">
								<el-link
									:type="scope.row.status > 2 || scope.row.order_status == -1 ? 'danger' : 'success'">
									{{ Constant.OrderStatus ? Constant.OrderStatus[scope.row.status] : "--" }}
								</el-link>
							</template>
						</el-table-column>
						<el-table-column :label="item.name" min-width="150"
							v-else-if="item.val && item.key == 'remark'">
							<template slot-scope="scope">
			...
			`${$t('order.membershipFree')}${scope.row.freeTime}${$t('public.minute')}`
											}}</span>
									</el-link>
						...
						<el-table-column :label="item.name" width="155" v-else-if="item.val && item.key == 'lose'">
							<template slot-scope="scope">
								<div>{{ scope.row.lose ? lostTitle[scope.row.lose] : '' }}</div>
							</template>
						</el-table-column>
						<el-table-column :label="item.name" :prop="item.key" :width="item.width || 120"
							v-else-if="item.val"></el-table-column>
					</template>
					<el-table-column :label="`${$t('public.operate')}`" width="165"
			...
			<el-dialog :visible.sync="dialogStatus" :center="true" :show-close="false" width="560px">
				<div class="mt-5 text-center text-black fs-c1 text-initial" slot="title">{{ dialogTitle[dialogType] }}
				</div>
				...
				<template v-if="[5, 7, 8, 9, 10, 11].indexOf(dialogType) > -1">
					<el-form class="custom-form pl-20 pr-20" label-width="auto">
						<el-form-item :label="`${$t('public.orderNo')}`">
							<el-input v-model="dform.orderNo"></el-input>
							<div class="mt-10 text-danger line-default" v-if="dialogType == 8">{{ $t('order.message') }}
							</div>
							<div class="mt-10 text-danger line-default" v-else-if="dialogType == 9">{{
			$t('order.message1') }}</div>
							<div class="mt-10 text-danger line-default" v-else-if="dialogType == 10">{{
			$t('order.message2') }}</div>
						</el-form-item>
			...
			$t('order.operateText') }}</el-button>
				...
				<div class="mt-30 text-center">
					<el-button size="medium" class="bg-body" @click="dialogStatus = false">{{ $t('public.cancel')
						}}</el-button>
					...
					<div class="pl-20 pr-20 text-black">
						...
						<div class="flex align-center pb-20 l-b">
							...
							<div class="pl-20">
								...
								<div class="flex mt-10">
									...
									<div v-if="isBrand() || isSaas()">{{ curRow.userMobile || `${$t('public.notHave')}`
										}}</div>
						...
						<div class="pb-20 l-b">
							...
							<div class="flex mb-10">
								...
								<div class="flex align-center">
									<el-avatar :src="payChannel[curRow.payType].logo" :size="15"
										v-if="payChannel[curRow.payType]" class="mr-5 radius-15"></el-avatar>
									<span
										v-if="curRow.orderAmount > 0 && curRow.feeType == 3 && (isBrand() || isSaas())">({{
			formatCurrency(curRow.orderAmount) }})</span>
							...
							<div class="flex mb-10">
								...
								<div class="text-cut">
									<el-tooltip :content="showFeeMode(curRow.feeType, curRow.feeMode, 2)"
										placement="top">
						...
						<el-form class="custom-form" label-width="130px" label-position="left">
							...
							<el-form-item :label="`${$t('order.returnTime')}:`">
								<el-date-picker v-model="dform.chargeEndTime" type="datetime"
									value-format="yyyy-MM-dd HH:mm" :picker-options="pickerOptionsEnd"
					...
					<div class="pl-20 pr-20 text-black">
						...
						<el-form class="custom-form" label-width="auto">
							...
							<el-form-item :label="`${$t('order.reasonForRefund')}:`">
								<el-input v-model="dform.reason"
									:placeholder="`${$t('order.reasonForRefund')}`"></el-input>
				...
				<template v-if="dialogType == 6">
					<div class="pl-20 pr-20 text-black">
						...
						<div class="mt-20 mb-15">
							{{ $t('public.orderInformation') }}
							<el-tag class="ml-10"
								:type="curRow.status > 2 || curRow.status == -1 ? 'danger' : 'success'" size="mini"
								effect="dark">
			...
			formatCurrency(curRow.orderAmount) }})</span>
									...
									<div class="flex">
										...
										<div>
											<span class="mr-5" v-if="curRow.afterLevel >= 0 || curRow.level >= 0">{{
			curRow.afterLevel ?
				$t('order.powerConsumption') : $t('order.DuringLease') }}({{
			...
			`${$t('order.membershipFree')}${curRow.freeTime}${$t('public.minute')}`
													}}</span>
											</template>
						...
						<template v-if="dform.orderDivide && dform.orderDivide.length > 0">
							...
							<el-table border :data="dform.orderDivide" :span-method="fenRunSpanMethod" class="custom">
								...
								<el-table-column width="120" :label="`${$t('public.dividedAmount')}`" align="center">
									<template slot-scope="scope">
										<span>{{ formatCurrency(accSub(scope.row.amount, scope.row.loseAmount))
											}}</span>
										<span v-if="scope.row.costAmount > 0">({{ $t('order.overtimeCosts') }}{{
			formatCurrency(scope.row.costAmount) }}</span>
									</template>
								</el-table-column>
								...
								<el-table-column :label="$t('order.splitStates')" align="center">
									<template slot-scope="scope">
										{{ scope.row.dividerPaymentStatus == 0 ? `${$t('order.status')}` :
			scope.row.dividerPaymentStatus ==
				-1 ? `${$t('order.status1')}` :
				scope.row.dividerPaymentStatus == -2 ? `${$t('order.status2')}` :
					`${$t('order.status3')}` }}
									</template>
								</el-table-column>
								<el-table-column width="120" :label="`${$t('public.refundAmount')}`" align="center">
									<template slot-scope="scope">
...
export default {
	...
	computed: {
		...
		defaultColumn() {
			...
			},
			{
				key: 'lose',
				val: false,
				hidden: !((this.checkAbility(['_DD_END', '_DD_HIDE', '_DD_RATIO', '_DD_TIME', '_DD_FAIL'], 1) && this
					.isBrand()) || this.isSaas()),
				name: this.$t('steal.ddStatus')
			}
			]
		},
		lostTitle() {
			return {
				BL: this.$t('steal.proportionalOmission'),
				HIDE: this.$t('steal.hideMissingOrders'),
				TIME_LIMIT: this.$t('steal.splitTimeLimit'),
				TIME: this.$t('steal.timeDeduction'),
				FAIL: this.$t('steal.noDepositToBePaid')
			}
		}
	},
	data() {
	...
	mounted() {
		...
		this.$get('/iot-saas-pay/admin/pay/channel/getQueryPayTypes',).then(res => {
			...
			this.payChannellist.unshift(obj);
						...
						this.$get('iot-saas-order/admin/order/detail', {
							...
							if (res.devicePopupRecordFeignOutFeign && res.status != 'F') {
								...
								if (res.devicePopupRecordFeignOutFeign.afterLevel > 0) {
									this.$set(this.curRow, 'afterLevel', accSub(res.devicePopupRecordFeignOutFeign.level, res
										.devicePopupRecordFeignOutFeign.afterLevel))
