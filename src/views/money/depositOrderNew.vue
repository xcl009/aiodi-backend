<template>
  <div>
    <el-row :gutter="20" type="flex" class="flex-wrap mb-15 stat-box text-nowrap text-white" v-show="listQuery.type == '1'">
      <el-col :span="24" :md="8" class="item-box">
        <div class="pl-30 pr-30 pt-20 pb-20 info radius-5">
          <div class="mb-15 text-white text-bold">超时已完成</div>
          <div class="flex">
            <div class="flex1">
      ...
      <el-col :span="24" :md="8" class="item-box">
        <div class="pl-30 pr-30 pt-20 pb-20 info radius-5">
          <div class="mb-15 text-white text-bold">超时待付款</div>
          <div class="flex">
            <div class="flex1">
              <div class="mb-5">总金额</div>
      ...
      <el-col :span="24" :md="8" class="item-box">
        <div class="pl-30 pr-30 pt-20 pb-20 info radius-5">
          <div class="mb-15 text-white text-bold">超时已退款</div>
          <div class="flex">
            <div class="flex1">
              <div class="mb-5">总金额</div>
    ...
    <el-row :gutter="20" type="flex" class="flex-wrap mb-15 stat-box text-nowrap text-white" v-show="listQuery.type == '2'">
      <el-col :span="24" :md="16" class="item-box">
        <div class="pl-30 pr-30 pt-20 pb-20 info radius-5">
          <div class="mb-15 text-white text-bold">待付款</div>
          <div class="flex">
            <div class="flex1">
      ...
      <el-col :span="24" :md="8" class="item-box">
        <div class="pl-30 pr-30 pt-20 pb-20 info radius-5">
          <div class="mb-15 text-white text-bold">进行中</div>
          <div class="flex">
            <div class="flex1">
              <div class="mb-5">可扣总额</div>
    ...
    <el-row :gutter="20" type="flex" class="flex-wrap mb-15 stat-box text-nowrap text-white" v-show="listQuery.type == '3'">
      <el-col :span="24" :md="16" class="item-box">
        <div class="pl-30 pr-30 pt-20 pb-20 info radius-5">
          <div class="mb-15 text-white text-bold">押金统计</div>
          <div class="flex">
            <div class="flex1">
      ...
      <el-col :span="24" :md="8" class="item-box">
        <div class="pl-30 pr-30 pt-20 pb-20 info radius-5">
          <div class="mb-15 text-white text-bold">租金统计</div>
          <div class="flex">
            <div class="flex1">
              <div class="mb-5">租借总额</div>
    ...
    <condition ref="condition" :clickSubmit="clickSubmit" @reset="reset" @query="toQuery">
      <template v-slot:tabs>
        <div class="mb-10 flex align-center bg-white">
          <div class="mr-10">类型</div>
          <el-tabs class="flex1" v-model="listQuery.type" @tab-click="toQuery()">
            <el-tab-pane :label="item" :name="''+index+''" v-for="(item, index) in {'1': '超时订单', '2': '预授权订单', '3': '预存订单'}" />
          </el-tabs>
        </div>
      ...
      <template v-slot:defult>
        ...
        <el-form-item label="日期">
          <el-date-picker class="range-day flex align-center" v-model="listQuery.date" type="daterange" range-separator="-"
      ...
      <el-table class="custom" id="table_box" ref="table_box" v-loading="listLoading" :data="list"
        ...
        <template v-for="item in showColumn" v-if="item.val">
          ...
          <!-- <el-table-column :label="item.name" v-else-if="item.key == 'rStatus'">
            <template slot-scope="scope">
              {{ scope.row.orderAmount > accAdd(scope.row.amountEnable, scope.row.amount) ? $t('public.refunded') : $t('public.unreimbursed') }}
            </template>
          </el-table-column> -->
          ...
          <el-table-column :label="item.name" :prop="item.key" :width="item.width || 120" v-else-if="!item.type"></el-table-column>
        </template>
      </el-table>
...
export default {
        ...
        disabledDate: (time) => {
          ...
          if (timeOptionRange) {
            return (time.getTime() > timeOptionRange.getTime() + secondNum || time.getTime() < timeOptionRange
              .getTime() - secondNum) || time.getTime() > Date.now() || time.getTime() < 1746028800000
          }
          return time.getTime() > Date.now() + 86400000 || time.getTime() < 1746028800000
        },
        onPick: (time) => {
          //当第一时间选中才设置禁用
  ...
  mounted(options) {
    let daysAgo = this.currentTime() - 31 * 86400
    this.listQuery.date = [
      this.parseTime(daysAgo < 1746028800 ? 1746028800 : daysAgo, '{y}-{m}-{d}'),
  ...
  methods: {
    ...
    getTotalStat() {
      ...
      if (params.date && params.date.length > 0) {
        params.chargeStartTime = params.date[0] + ' 00:00:00'
        params.chargeEndTime = params.date[1] + ' 23:59:59'
        delete params.date
      }
      delete params.page
      delete params.size
      ...
      this.$get(`iot-saas-order/admin/deposit/statistics/sum`, params).then(res => {
        this.totalStat = res || {}
      })
    },
    ...
    getList() {
      ...
      this.$get('iot-saas-order/admin/deposit/statistics/list', params).then(async (res = {}) => {
