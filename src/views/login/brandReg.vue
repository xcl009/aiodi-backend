<template>
  <el-row type="flex" align="middle" class="login-container flex-wrap">
    ...
    <div class="login-form bg-white radius-10">
      <div class="p-20 fs-b2 text-center text-black" v-if="clientWidth < 992">邀请注册</div>
      <div class="mb-20 p-30 fs-b2 text-center text-black" v-else>邀请注册</div>
      <div class="mb-30 pb-20">
        <el-steps :space="200" :active="steps" align-center>
          <el-step title="填写基本信息"></el-step>
          <el-step title="选择经营品类"></el-step>
          <el-step title="完成注册"></el-step>
        </el-steps>
      </div>

      <el-form ref="form" :rules="rules" :model="form" :hide-required-asterisk="true" :label-width="clientWidth < 992 ? '' : '90px'" label-position="left" v-if="steps == 0">
        <el-row class="pl-50 pr-50 custom-form" :class="{'mobile': clientWidth < 992}" :gutter="50">
          <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
            <el-form-item ref="name" label="用户名" prop="name">
              <el-input v-model="form.name" placeholder="5-20个字符，可用字母、数字等" />
            </el-form-item>
            <el-form-item ref="password" label="登录密码" prop="password">
              <el-input type="password" v-model="form.password" placeholder="8-16个字符" show-password	/>
            </el-form-item>
            <el-form-item ref="confirmPassword" label="确认密码" prop="confirmPassword">
              <el-input type="password" v-model="form.confirmPassword" placeholder="请再次输入密码" show-password	@blur="checkPwd()"/>
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
            <el-form-item ref="invitationCode" label="邀请码" prop="invitationCode">
              <el-input v-model="form.invitationCode" placeholder="请填写邀请码" />
            </el-form-item>
            <el-form-item ref="mobile" label="手机号码" prop="mobile">
              <el-input v-model="form.mobile" placeholder="请输入手机号码" />
            </el-form-item>
            <el-form-item ref="verificationCode" label="验证码" prop="verificationCode">
              <el-input v-model="form.verificationCode" placeholder="请输入验证码">
                <template slot="append"><auth-code ref="authCode" :isBtn="false" @authCode="getAuthCode()" class="text-primary"></auth-code></template>
              </el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <div class="pb-30 text-center line-1" :class="{'mt-pc': clientWidth >= 992, 'mt-30': clientWidth < 992}">
          <el-button type="primary" class="login-btn" @click="checkSubmit()" :disabled="clickSubmit">下一步</el-button>
          <div class="mt-20 flex align-center justify-center">
            <el-checkbox v-model="form.checked"></el-checkbox>
            <span class="ml-5 text-gray">已阅读并同意</span>
            <a class="text-primary" @click="dialogStatus = true">《品牌方注册协议》</a>
          </div>
        </div>
      </el-form>
      ...
      <div v-if="steps == 1">
        <div class="flex justify-center flex-wrap" style="padding: 30px 100px;">
          <el-button size="medium" :type="form.selCode === item.code ? 'primary' : ''"
            ...
            @click="form.selCode = item.code">{{ item.name }}</el-button>
        </div>
        <div class="pb-30 text-center line-1" :class="{'mt-pc': clientWidth >= 992, 'mt-30': clientWidth < 992}">
          <div>
            <el-button type="info" class="login-btn btn-body" @click="steps = 0">上一步</el-button>
            <el-button type="primary" class="login-btn" @click="onSubmit()" :disabled="clickSubmit">完成注册</el-button>
          </div>
        </div>
      </div>
      ...
      <div v-if="steps == 2">
        <div class="flex justify-center">
          ...
          <div class="pl-15 pt-5">
            <div class="fs-c1">恭喜您，注册成功</div>
            <div class="mt-5 text-gray">请妥善保存您的账号信息</div>
          </div>
        </div>
    ...
    <el-dialog :visible.sync="dialogStatus" :center="true" :close-on-click-modal="false" :show-close="false" width="650px">
      <div class="mt-5 text-center text-black fs-c1 text-initial" slot="title">品牌方注册协议</div>
      <div class="p-10" style="max-height: 500px;  min-height: 200px;  overflow-y: scroll; background-color: #f5f5f5;">
        协议内容
      </div>
      <div class="mt-30 text-center">
        <el-button size="medium" type="primary" @click="dialogStatus = false; form.checked = true">确定</el-button>
      </div>
    </el-dialog>
  </el-row>
</template>
