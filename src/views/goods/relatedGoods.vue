    ...
    <div class="pl-20 pr-20 pt-20 pb-5 bg-white">
      <el-form ref="form" label-width="auto">
        <el-table class="ptd-5" id="list_table" ref="list_table" :data="list">
          <el-table-column :label="$t('goods.hatch')" width="150">
            <template slot-scope="scope">
              {{ scope.row.position }}
            </template>
          </el-table-column>
          <el-table-column :label="$t('goods.commodity')">
            <template slot-scope="scope">
              <div class="flex align-center">
                <el-avatar shape="square" :size="50" :src="scope.row.picture" fit="fill" icon="el-icon-picture-outline"
                  v-if="scope.row.picture"></el-avatar>
    ...
    <el-dialog :visible.sync="dialogStatus" :center="true" :show-close="false" width="750px">
      ...
      <template v-if="dialogType == 1">
        <el-table class="ptd-5" :data="goodsList" border>
          <el-table-column :label="$t('public.image')" width="80">
            <template slot-scope="scope">
              <div class="flex">
                <el-avatar shape="square" :size="50" :src="scope.row.picture" fit="fill"
      ...
      <div class="pb-20 mt-30 text-center">
        ...
        <el-button size="medium" type="primary" @click="dialogConfirm()" :disabled="clickSubmit" v-if="dialogType != 1">{{
