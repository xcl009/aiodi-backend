  ...
  <div>
    <condition ref="condition" :clickSubmit="clickSubmit" @reset="reset" @query="toQuery" :exportStatus="true"
      ...
      <template v-slot:defult>
        <el-form-item label="合作方式">
          <el-select :placeholder="$t('public.pleaseSelect')" v-model="listQuery.cooperationMode" @change="toQuery()">
            <el-option :label="$t('public.all')" :value="''" />
            <el-option :label="i" :value="k" v-for="(i, k) in cooperationMode" />
          </el-select>
        </el-form-item>
        <el-form-item label="状态">
          <el-select :placeholder="$t('public.pleaseSelect')" v-model="listQuery.status" @change="toQuery()">
            <el-option :label="$t('public.all')" :value="''" />
        ...
        <el-form-item label="负责人">
          <el-input :placeholder="$t('public.enter')" v-model="form.contactCerson" />
        </el-form-item>
      </template>
    </condition>
    ...
    <div class="pl-15 pr-15 pb-5 bg-white">
      <el-table class="custom" id="list_table" ref="list_table" v-loading="listLoading" :data="list"
        ...
        <el-table-column label="合作方式" width="120">
          <template slot-scope="scope">
            {{ cooperationMode[scope.row.cooperationMode] }}
          </template>
        </el-table-column>
        <el-table-column label="称呼" width="150">
          <template slot-scope="scope">
            {{ scope.row.name }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="状态" width="120">
          <template slot-scope="scope">
            <a class="cursor text-primary" @click="setRows(3, scope.row, 2)">{{ statusObj[scope.row.status] || '--' }}</a>
          </template>
        </el-table-column>
        <el-table-column label="负责人" width="120">
          <template slot-scope="scope">
            <a class="cursor text-primary" @click="setRows(3, scope.row, 1)">{{ scope.row.contactPerson || '无' }}</a>
          </template>
        </el-table-column>
        ...
        <el-table-column label="报价内容" min-width="250">
          <template slot-scope="scope">
    ...
    <xlsx ref="toXlsx" fileName="报价记录"></xlsx>
