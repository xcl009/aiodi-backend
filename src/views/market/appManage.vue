            ...
            <div class="mt-15 flex align-center">
              <template v-for="(sitem, idx) in item.priceSettings">
                <div class="flex1" v-if="idx == 0">
                  <span class="fs-b3 text-danger">{{ sitem.monthAmount > 0 ? `¥${sitem.monthAmount}` : sitem.yearAmount >
                    0 ? `¥${sitem.yearAmount}` : `¥${sitem.permanentAmount}` }}</span>
                  <span class="text-grey">{{ sitem.monthAmount > 0 ? `/${$t('payType.monthlyPayment')}` : sitem.yearAmount
                    > 0 ? `/${$t('payType.annualPayment')}` : `/${$t('payType.permanent')}` }}</span>
                </div>
              </template>
              <el-button type="primary" size="medium" @click="$router.push({ path: `/market/addApp?id=${item.id}` })">{{
                $t('public.modifyNow') }}</el-button>
              <el-button type="danger" @click="setRows(1, item, 1)" v-if="brandId">{{ $t('public.giftService')
              }}</el-button>
            </div>
          </div>
        </el-col>
        ...
        <div class="flex justify-center">
          <el-form class="custom-form pl-20 pr-20" label-width="auto">
            ...
            <el-form-item :label="$t('public.endTime')">
              <el-date-picker v-model="dform.giveEndDatetime" type="date" value-format="yyyy-MM-dd"
