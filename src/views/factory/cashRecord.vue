  ...
  <div>
    <condition ref="condition" :clickSubmit="clickSubmit" @reset="reset" @query="toQuery">
      <template v-slot:defult>
        ...
        <el-form-item :label="$t('goods.id')">
          <el-input v-model="form.id" :placeholder="$t('goods.id')" />
        </el-form-item>
        <el-form-item :label="$t('public.withdrawalTime')">
          <el-date-picker class="range-day flex align-center" v-model="form.date" type="daterange"
            value-format="timestamp" range-separator="-" :start-placeholder="$t('public.statrtDate')"
    ...
    <div class="mt-15">
      <div class="flex bg-white">
        <div class="pl-15 pr-15 flex1">
          <el-tabs v-model="listQuery.apply_status" class="mb-0" @tab-click="typeClick">
            ...
            <el-tab-pane :label="`${$t('public.applying')}`" name="1" />
            <el-tab-pane :label="`${$t('public.rejected')}`" name="3" />
            <el-tab-pane :label="`${$t('public.passed')}`" name="2" />
          </el-tabs>
        </div>
      </div>
      <div class="pl-15 pr-15 pb-5 bg-white">
        <el-table class="ptd-5" id="list_table" ref="list_table" v-loading="listLoading" :data="list"
          ...
          <el-table-column :label="$t('public.withdrawalTime')" width="90">
            <template slot-scope="scope">
          ...
          <el-table-column :label="$t('goods.payee')" width="220">
            <template slot-scope="scope">
    ...
    <el-dialog :visible.sync="dialogStatus" :center="true" :close-on-click-modal="false" :show-close="false"
      ...
      <template v-if="dialogType == 1">
        <div class="mb-20 text-center">{{ $t('goods.text') }}</div>
        <el-form class="pt-5 custom-form">
          <el-input v-model="dform.remark" :placeholder="$t('goods.inputText')" type="textarea" :rows="5" />
        </el-form>
      </template>
      <template v-if="dialogType == 2">
        <div class="mb-20 text-center">{{ $t('goods.text1') }}</div>
        <el-form class="pt-5 custom-form">
          <el-input v-model="dform.remark" :placeholder="$t('goods.inputText')" type="textarea" :rows="5" />
        </el-form>
      </template>
