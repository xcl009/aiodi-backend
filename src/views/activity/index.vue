    ...
    <el-row :gutter="20" type="flex" class="flex-wrap mb-15 fs-s2 text-nowrap">
      <el-col :span="24" :md="5">
        <div class="flex align-center pl-15 pr-15 pt-10 pb-10 bg-white radius-5">
          ...
          <div class="flex1 ml-10">
            <div><span class="mr-5 fs-b3 text-bold text-black">{{ listTotal || 0 }}</span>{{ $t('public.individual') }}
            </div>
      ...
      <el-col :span="24" :md="5">
        <div class="flex align-center pl-15 pr-15 pt-10 pb-10 bg-white radius-5">
          ...
          <div class="flex1 ml-10">
            <div><span class="mr-5 fs-b3 text-bold text-black">{{ totalStat.deviceStockLifeStatusCounts || 0 }}</span>{{
      ...
      <el-col :span="24" :md="5">
        <div class="flex align-center pl-15 pr-15 pt-10 pb-10 bg-white radius-5">
          ...
          <div class="flex1 ml-10">
            <div><span class="mr-5 fs-b3 text-bold text-black">{{ totalStat.deviceStockLifeLendingStatusCounts || 0
            }}</span>{{ $t('public.individual') }}</div>
      ...
      <el-col :span="24" :md="5">
        <div class="flex align-center pl-15 pr-15 pt-10 pb-10 bg-white radius-5">
          ...
          <div class="flex1 ml-10">
            <div><span class="mr-5 fs-b3 text-bold text-black">{{ totalStat.deviceStockLifePopupStatusCounts || 0
            }}</span>{{ $t('public.individual') }}</div>
      ...
      <el-col :span="24" :md="5">
        <div class="flex align-center pl-15 pr-15 pt-10 pb-10 bg-white radius-5">
          <img :src="require('@/assets/lease/wait.svg')" width="54">
          <div class="flex1 ml-10">
            <div><span class="mr-5 fs-b3 text-bold text-black">{{ totalStat.deviceStockLifeExceptionStatusCounts || 0
            }}</span>{{ $t('public.individual') }}</div>
      ...
      <el-table class="custom" id="list_table" ref="list_table" v-loading="listLoading" :data="list"
        ...
        <template v-for="item in showColumn" v-if="item.val">
          <el-table-column :label="item.name" min-width="140" v-if="item.key == 'name'">
            <template slot-scope="scope">
              <div class="flex align-center">
                <div :class="`role-label fs-s2 ` + (scope.row.createType == 1 ? 'agent' : '')">{{ scope.row.createType ==
                  1 ? $t('public.take') : $t('public.discuss') }}</div>
          ...
          <el-table-column :label="item.name" width="150" v-else-if="item.key == 'onlineStatue'">
            <template slot-scope="scope">
              <div class="flex align-center">
                <el-button :type="onlineStatus[scope.row.onlineStatue] ? onlineStatus[scope.row.onlineStatue].type : ''"
                  circle class="mr-5 btn-status_circle"></el-button>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$post('iot-saas-activity/admin/v2/activity/findPage', params).then(async (res = {}) => {
        let list = res.rows || []
        if (list.length > 0) {
