              ...
              <div class="flex">
                ...
                <div class="flex1">
                  <template v-for="key in ['today', 'yesterday', 'week', 'lastWeek', 'month', 'lastMonth']">
                    <div class="progress">
                      <el-progress :stroke-width="12" :show-text="false"
                        :percentage="index == 0 ? querHistogram[key].amountPP : index == 1 ? querHistogram[key].orderPP : querHistogram[key].unitPP"></el-progress>
                    </div>
                  </template>
                </div>
                <div class="label">
          ...
          formatter: params => {
            if (params.seriesName !== 'mouseoutSeries' && params.seriesName !== 'pie2d') {
              return `${params.seriesName}<br/>` +
                `<span style="display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:300px;background-color:${params.color};"></span>` +
